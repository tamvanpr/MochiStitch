...
class MochiStitchSettingsRepository(private val dataStore: DataStore<Preferences>) {
    ...
    val settingsFlow: Flow<MochiStitchSettings> = dataStore.data.map { prefs ->
        MochiStitchSettings(
            ...
            readingDirection = prefs[KEY_READING_DIRECTION]?.let { runCatching { ReadingDirection.valueOf(it) }.getOrNull() } ?: ReadingDirection.VERTICAL,
            ...
            mochiSmartSensitivity = prefs[KEY_MOCHI_SMART_SENSITIVITY]?.let { runCatching { DetectionSensitivity.valueOf(it) }.getOrNull() } ?: DetectionSensitivity.HIGH,
            ...
            pixelComparisonSensitivity = prefs[KEY_PIXEL_COMPARISON_SENSITIVITY] ?: 0.65f,
            pixelComparisonMargins = prefs[KEY_PIXEL_COMPARISON_MARGINS] ?: 0,
            pixelComparisonStep = prefs[KEY_PIXEL_COMPARISON_STEP] ?: 3,
            pixelComparisonMaxDeviationFactor = prefs[KEY_PIXEL_COMPARISON_MAX_DEVIATION_FACTOR] ?: 0.15f,
